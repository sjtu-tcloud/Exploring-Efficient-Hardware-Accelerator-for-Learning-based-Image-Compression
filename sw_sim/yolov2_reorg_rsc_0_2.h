...
template <typename T>
int diff_set(T *in0, T *in1, int data_num, double err_max, int idx, bool init_en, bool exit_en, bool *prv_exit_en){

    float tmp_dataQ = pow(0.5, 14);

    static int prv_exit_val;
    static int idx_val;
    static double diff_sum_min, diff_max_min, diff_sum_prv;
    if(init_en){
        idx_val = -1;
        diff_sum_min = 65536;
        prv_exit_val = 0;
    ...
    }else if(diff_sum > diff_sum_prv){
        prv_exit_val++;
    }

    diff_sum_prv = diff_sum;
