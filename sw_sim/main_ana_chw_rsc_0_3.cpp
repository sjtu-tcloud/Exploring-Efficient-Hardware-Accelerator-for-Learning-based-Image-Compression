...
#include <time.h>
#include <sys/time.h>
#include "yolov2_reorg_rsc_0_2.h"
...
int main( int argc, char *argv[])
{
	...
	memset(ifm_sqQarray, 0, sizeof(ifm_sqQarray));
	int interQarray[LNUM];
	memset(interQarray, 0, sizeof(interQarray));
	...
	for(int lnum = 0; lnum < LNUM; lnum++){
		...
		uint8_t ifm_sqQ = 0;//0-28, Q14*2
		uint8_t interQ = 0;//0-28
		bool ifm_sqQ_en = 0;
		bool interQ_en = 0;
		...
		ifm_sqQ_en = 1;
		bool prv_exit_en = 0;
		for(int tmpQ=0; tmpQ < 7+1; tmpQ++){
			IC_codec(inout_fixed_buf, ifm_ptr, kernel_w_ptr, bias_ptr, ifm_Scale_ptr, ofm_Scale_ptr, kernel_Scale_ptr,
						ifm_num, ih, iw, ofm_num, oh, ow, pad, ksize, kstride, ltype,
						p_stride, c_stride, wb_mode, ps_sf_bit, ps_mask, sq_en, TR, TC, TM, TN, OHW, KxK,
						IHxIW, p_stridexIR, p_stridexIC, bias_en, NToy, NTox, NTof, NTcomb, NTif, lmode, NTcomb_l,
						tmpQ, interQ, ifm_sqQ_en, interQ_en);

			ifm_sqQ = diff_set(ofm_ptr, inout_fixed_buf, oh*ow*ofm_num, 1e-5, tmpQ, tmpQ==0, tmpQ==7, &prv_exit_en);
			if(prv_exit_en)
				break;
		}

		interQ_en = 1;
		uint8_t maxQ = 28 - ifm_sqQ;
		printf("maxQ = %d\n", maxQ);
		prv_exit_en = 0;
		for(int tmpQ=0; tmpQ < maxQ+1; tmpQ++){
			IC_codec(inout_fixed_buf, ifm_ptr, kernel_w_ptr, bias_ptr, ifm_Scale_ptr, ofm_Scale_ptr, kernel_Scale_ptr,
						ifm_num, ih, iw, ofm_num, oh, ow, pad, ksize, kstride, ltype,
						p_stride, c_stride, wb_mode, ps_sf_bit, ps_mask, sq_en, TR, TC, TM, TN, OHW, KxK,
						IHxIW, p_stridexIR, p_stridexIC, bias_en, NToy, NTox, NTof, NTcomb, NTif, lmode, NTcomb_l,
						ifm_sqQ, tmpQ, ifm_sqQ_en, interQ_en);

			interQ = diff_set(ofm_ptr, inout_fixed_buf, oh*ow*ofm_num, 1e-5, tmpQ, tmpQ==0, tmpQ==maxQ, &prv_exit_en);
			if(prv_exit_en)
				break;
		}
		...
		interQarray[lnum] = interQ;
		ifm_sqQarray[lnum] = ifm_sqQ;
	...
	for(int ow=0; ow<ofm_w;ow++){
		ofm_tmp_buf[oc*ofm_h*ofm_w + oh*ofm_w + ow] = ofm_ptr[((oc/LANE_NUM)*ofm_h*ofm_w + oh*ofm_w + ow)*LANE_NUM + (oc & (LANE_NUM-1))]
			*tmp_pow_out*ofm_Scale_ptr[((oc/LANE_NUM))*LANE_NUM + (oc & (LANE_NUM-1))];
	}

	FILE *fp_ioq;
	fp_ioq = fopen("ana_i16c_interQ.bin", "wb");
	if(!fp_ioq) printf("ana_i16c_interQ.bin\n");
	fwrite(interQarray, sizeof(int), LNUM, fp_ioq);
	fclose(fp_ioq);//iofm quanti

	fp_ioq = fopen("ana_i16c_ifm_sqQ.bin", "wb");
	if(!fp_ioq) printf("ana_i16c_ifm_sqQ.bin\n");
	fwrite(ifm_sqQarray, sizeof(int), LNUM, fp_ioq);
	fclose(fp_ioq);//iofm quanti
