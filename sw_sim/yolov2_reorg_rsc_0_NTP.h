...
void ana_a05_chw(int32_t *in0, float* in1, int h, int w, int c){

	for(int k = 0; k < c; ++k)
        for(int j = 0; j < h; ++j)
            for(int i = 0; i < w; ++i){
                int index = k*h*w + i + w*j;
                in0[index] = floor(in1[index] + 0.5);
    }
}
...
int quantize_ifm_i8c_scale(float *in,float *out,int wh_num, int c_num, int lane_num, float *scale_array)
{

const float MIN_VALUE = (-1024*1024*1024);
const float MAX_VALUE = (1024*1024*1024);

	printf("Layer[x]'s fm, c=%12d, LANE_ext(c_num)=%12d, hw=%12d ", c_num, LANE_ext(c_num), wh_num);
	for(int j=0; j<LANE_ext(c_num); j++){
	    float max_abs = 0.;
        if(j<c_num){
            for(int i=0; i<wh_num; i++)
            {
                float tmp_in_float = in[((j/lane_num)*wh_num + i)*lane_num + (j & (lane_num-1))];
                // tmp_in_float = fabs(tmp_in_float);
        ...
        // printf("[c:%d]float scale =%5.7lf\n",j, scale_array[j]);//find float min max
    ...
    for(int i=0; i<wh_num; i++)
	{
        ...
        // assert(tmp_in_float <= scale_array[j]);
        assert(scale_array[j]>0);
        float tmp_in_f_s = tmp_in_float/scale_array[j];
        // if(!((tmp_in_f_s <= 1.0) && (tmp_in_f_s >= -1.0))){
        //     printf("\ntmp_in_float %5.5lf / scale_array[j] %5.5lf\n", tmp_in_float, scale_array[j]);
        // }
        assert((tmp_in_f_s <= 1.0) && (tmp_in_f_s >= -1.0));

        int8_t tmp_fixed = (int8_t)(tmp_in_f_s*pow(2.0,maxQ));
		float tmp_out_f_s = (float)tmp_fixed*pow(0.5,maxQ);
        assert((tmp_out_f_s <= 1.0) && (tmp_out_f_s >= -1.0));
