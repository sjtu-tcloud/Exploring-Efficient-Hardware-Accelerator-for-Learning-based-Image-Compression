...
//calc ifm/ofm width/height
	...
	memset(ifm_Scale, 0, ifm_channel_num*sizeof(float));
	memset(ofm_Scale, 0, ofm_channel_num*sizeof(float));
	...
	FILE *fp_ioq;
	fp_ioq = fopen("ana_i16c_ifm_scale.bin", "wb");
	if(!fp_ioq) printf("ana_i16c_ifm_scale.bin\n");
