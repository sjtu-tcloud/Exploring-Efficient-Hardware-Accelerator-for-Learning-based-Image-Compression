...
void codec_ofm_store_chw_tile(int16_t *ofm, float ofm_buf[Tmax_dx][MAX_Trc][LANE_NUM], int16_t *ifm_extra, float b_buf[((MAX_BETA_LENGTH+LANE_NUM-1)/LANE_NUM)][LANE_NUM],
    ...
    float *ifm_scale, float *ofm_scale, float *kernel_scale, uint8_t ifm_sqQ, uint8_t interQ, bool ifm_sqQ_en, bool interQ_en){
//mode=0 normal, 1=src0*sqrt(src1), 2=src0/sqrt(src1)
	...
	for(uint32_t tcb = 0; tcb < Tcomb_TC; tcb++){
		...
		for(int tbl_min=0; tbl_min < burstlen; tbl_min++){
			...
			for(uint32_t x=0; x<LANE_NUM; x++){
                if(interQ_en){
                    ...
                    tmp_f32i[x] = post_process(f2i32(ofm_buf[tm][ofm_idx][x])*pow(0.5, ComQ)*kernel_scale[tm*LANE_NUM+m+x], b_buf[m/LANE_NUM + tm][x],
                                                    tmp_ifm_ex[x]*pow(0.5, 14)*ifm_scale[tm*LANE_NUM+m+x], mode, bias_en)
                                                    /ofm_scale[tm*LANE_NUM+m+x]*pow(2.0, 14);
                }else{
                    // tmp_f32i[x] = post_process(ofm_buf[tm][tr*TC_MIN + tc][x], bias[m + tm*LANE_NUM + x], tmp_ifm_ex[x], mode, bias_en);
                    tmp_f32i[x] = post_process(ofm_buf[tm][ofm_idx][x]*kernel_scale[tm*LANE_NUM+m+x], b_buf[m/LANE_NUM + tm][x],
                                                    tmp_ifm_ex[x]*pow(0.5, 14)*ifm_scale[tm*LANE_NUM+m+x], mode, bias_en)
                                                    /ofm_scale[tm*LANE_NUM+m+x]*pow(2.0, 14);
                    // tmp_f32i[x] = post_process(ofm_buf[tm][tr*TC_MIN + tc][x], local_bias_buffer[tm][x], tmp_ifm_ex[x], mode, bias_en);
                }
			}
		...
		// 	for(uint32_t x=0; x<LANE_NUM; x++){
        //         // tmp_f32i[x] = post_process(ofm_buf[tm][tr*TC_MIN + tc][x], bias[m + tm*LANE_NUM + x], tmp_ifm_ex[x], mode, bias_en);
        //         tmp_f32i[x] = post_process(ofm_buf[tm][cnt[z]][x], b_buf[m/LANE_NUM + tm][x], tmp_ifm_ex[x], mode, bias_en);
        //         // tmp_f32i[x] = post_process(ofm_buf[tm][tr*TC_MIN + tc][x], local_bias_buffer[tm][x], tmp_ifm_ex[x], mode, bias_en);
		// 	}
        //     cnt[z] = cnt[z] + 1;
...
void load_kernel_reorg(int16_t k_buf[Tmax_dx][MAX_Tif][MAX_KxK][LANE_NUM], int16_t *kernel, uint8_t m, uint8_t n, uint8_t TM_MIN, uint8_t TN_MIN, uint8_t K, uint8_t KxK,
                        float *kernel_scale, float *ifm_scale, bool sq_en){
...
// #pragma HLS PIPELINE II=1
		...
		for(uint32_t x=0; x<LANE_NUM; x++){
            ...
            // if(sq_en)
			//     k_buf[t1][t2][t3][x] = tmp_f32i[x]*pow(0.5, 14)*kernel_scale[m + t1*LANE_NUM + x]
            //                                 /(ifm_scale[n+t2]*ifm_scale[n+t2]);
            // else
            //     k_buf[t1][t2][t3][x] = tmp_f32i[x]*pow(0.5, 14)*kernel_scale[m + t1*LANE_NUM + x]
            //                                 /ifm_scale[n+t2];

			// k_buf[t1][t2][t3][x] = tmp_f32i[x]*pow(0.5, 14);
            k_buf[t1][t2][t3][x] = tmp_f32i[x];

            // if(sq_en)
			//     k_buf[t1][t2][t3][x] = tmp_f32i[x]*pow(0.5, 14)*kernel_scale[ (m/LANE_NUM + t1)*LANE_NUM + x]
            //                                 /(ifm_scale[((n+t2)/LANE_NUM)*LANE_NUM + ((n+t2) % LANE_NUM)]*
            //                                 ifm_scale[((n+t2)/LANE_NUM)*LANE_NUM + ((n+t2) % LANE_NUM)]);
            // else
            //     k_buf[t1][t2][t3][x] = tmp_f32i[x]*pow(0.5, 14)*kernel_scale[ (m/LANE_NUM + t1)*LANE_NUM + x]
            //                         /ifm_scale[((n+t2)/LANE_NUM)*LANE_NUM + ((n+t2) % LANE_NUM)];
...
void load_ifm_pad_square(float ifm_buf[Tnax_dx][MAX_IB_HW][LANE_NUM], int16_t *ifm, uint16_t IR, uint16_t IC, uint8_t K, uint32_t IHxIW, uint8_t p_stride,
        ...
        float *ifm_scale, uint8_t ifm_sqQ, bool ifm_sqQ_en){
...
// #pragma HLS PIPELINE II=1
                ...
                if(ifm_sqQ_en){
                    for(uint32_t x=0; x<LANE_NUM; x++){
                        ...
                        int16_t tmp_out_sr_i16 = tmp_in >> ifm_sqQ;
                        tmp_out = tmp_out_sr_i16*pow(0.5, 14-ifm_sqQ);

                        ifm_buf[t1][ifm_idx][x] = tmp_out;
                    }
                }else{
                    for(uint32_t x=0; x<LANE_NUM; x++){
                        float tmp_in, tmp_out;
                        // tmp_in = tmp_f32i[x]*pow(0.5, 14)*ifm_scale[t1*LANE_NUM + x + n];
                        tmp_in = tmp_f32i[x]*pow(0.5, 14);//re-map to (-2, 2]
                        // tmp_in = tmp_f32i[x]*pow(0.5, 14);//because kernel's kernel_scale have combined ifm_scale
                        if(square_en&&(tmp_in<0)){
                            tmp_out = -tmp_in;
                        }
                        else
                        ...
                        ifm_buf[t1][ifm_idx][x] = tmp_out;
                        // input_buffer[t1][t2_idx*TCol_MIN_r + t3_idx][x] = tmp_f32i[x];
                    }
                }
...
void load_ifm_k_wrapper(float ifm_buf[Tnax_dx][MAX_IB_HW][LANE_NUM], int16_t k_buf[Tmax_dx][MAX_Tif][MAX_KxK][LANE_NUM], int16_t *ifm, int16_t *kernel,
        ...
        float *ifm_scale, float *kernel_scale, uint8_t ifm_sqQ, bool ifm_sqQ_en){
    ...
    load_ifm_pad_square( ifm_buf, ifm, IR, IC, K, IHxIW, p_stride, c_stride, pad, ps_sf_bit, ps_mask, sq_en,
                TN_MIN, TR_MIN, TC_MIN, n, r, c, lmode, r_init_en, c_init_en, TCol_lps_n, ifm_scale, ifm_sqQ, ifm_sqQ_en);
...
void Compute_wrapper_sub(float ifm_buf[Tnax_dx][MAX_IB_HW][LANE_NUM], float ofm_buf[Tmax_dx][MAX_Trc][LANE_NUM],
                    ...
                    uint8_t k_offset, uint32_t ofm_offset, uint8_t ifm_sqQ, uint8_t interQ, bool ifm_sqQ_en, bool interQ_en){
    ...
    if(interQ_en){
        ...
        float tmp_exp_ifm = pow(2.0, 14-ifm_sqQ);
        ...
        for(uint16_t tc=0; tc<TC_MIN; tc++){
            ...
            for(int tm=0; tm< MAX_Tof; tm++){
                if(ky==0&&kx==0&&init_en)
                    ...
                    // partial_sum[tm] = ofm_buf[tm][ofm_idx];
                    partial_sum[tm] = f2i32(ofm_buf[tm/LANE_NUM][ofm_idx][tm%LANE_NUM]);
                }
...
void conv2d_Acc_sub(float ifm_buf[Tnax_dx][MAX_IB_HW][LANE_NUM], float ofm_buf[Tmax_dx][MAX_Trc][LANE_NUM],
     int16_t k_buf[Tmax_dx][MAX_Tif][MAX_KxK][LANE_NUM],
     int K, int TCol_lps, uint8_t c_stride, bool init_en, uint32_t *TRxTC_M_offset, uint8_t mode, uint8_t subK_num,
     uint16_t *TR_MIN_sub, uint16_t *TC_MIN_sub, bool enable, uint8_t ifm_sqQ, uint8_t interQ, bool ifm_sqQ_en, bool interQ_en){//NHWC, kernel KKNM
    ...
    for(uint8_t z=0; z<subK_num; z++){
        ...
        Compute_wrapper_sub( ifm_buf, ofm_buf, k_buf, Ky_s[z], Kx_s[z], TR_MIN_sub[z], TC_MIN_sub[z], c_stride, TCol_lps,
                                ifm_offset_s[z], init_en, k_offset_s[z], TRxTC_M_offset[z], ifm_sqQ, interQ, ifm_sqQ_en, interQ_en);
    }

}

// void lc_wrapper(float *ifm, float *kernel, float ofm_buf[Tmax_dx][MAX_Trc][LANE_NUM],
...
//     static float ifm_buf0[Tnax_dx][MAX_IB_HW][LANE_NUM];
//     static float ifm_buf1[Tnax_dx][MAX_IB_HW][LANE_NUM];
//     static float k_buf0[Tmax_dx][MAX_Tif][MAX_KxK][LANE_NUM];
//     static float k_buf1[Tmax_dx][MAX_Tif][MAX_KxK][LANE_NUM];
...
//                 p_stride, c_stride, pad, ps_sf_bit, ps_mask, sq_en, lmode, r_init_en, c_init_en, TCol_lps_n0, in_flag);

//             // codec_conv2d_chw_tile( ifm_buf1, ofm_buf, k_buf1, K, c_stride, TX_MIN_n[1][1], TX_MIN_n[1][2], 0, TCol_lps_n1[0], proc_flag);
//             conv2d_Acc_sub(ifm_buf1, ofm_buf, k_buf1, K, TCol_lps_n1[0], c_stride, 1, TRxTC_M_offset_n[1], c_mode, subK_num, TR_MIN_sub_n[1], TC_MIN_sub_n[1], proc_flag);
...
//                 p_stride, c_stride, pad, ps_sf_bit, ps_mask, sq_en, lmode, r_init_en, c_init_en, TCol_lps_n1, in_flag);

//             // codec_conv2d_chw_tile( ifm_buf0, ofm_buf, k_buf0, K, c_stride, TX_MIN_n[0][1], TX_MIN_n[0][2], 0, TCol_lps_n0[0], proc_flag);
//             conv2d_Acc_sub(ifm_buf0, ofm_buf, k_buf0, K, TCol_lps_n0[0], c_stride, 1, TRxTC_M_offset_n[0], c_mode, subK_num, TR_MIN_sub_n[0], TC_MIN_sub_n[0], proc_flag);
...
//                     conv2d_Acc_sub(ifm_buf1, ofm_buf, k_buf1, K, TCol_lps_n1[0], c_stride, (tn_n[1]==0), TRxTC_M_offset, c_mode,
//                             subK_num, TR_MIN_sub, TC_MIN_sub, proc_flag);
...
//                     conv2d_Acc_sub(ifm_buf0, ofm_buf, k_buf0, K, TCol_lps_n0[0], c_stride, (tn_n[0]==0), TRxTC_M_offset, c_mode,
//                             subK_num, TR_MIN_sub, TC_MIN_sub, proc_flag);
...
void lc_wrapper(int16_t *ifm, int16_t *kernel, float ofm_buf[Tmax_dx][MAX_Trc][LANE_NUM],
                int IF, int IR, int IC, int OF, int OR, int OC, int pad, int K,
                uint8_t p_stride, uint8_t c_stride, uint8_t ps_sf_bit, uint8_t ps_mask, bool sq_en,
                uint16_t TR, uint16_t TC, uint16_t TM, uint16_t TN, uint8_t KxK, uint32_t IHxIW, uint16_t NTif,
                uint8_t lmode, uint16_t m, uint16_t r, uint16_t c, bool r_init_en, bool c_init_en, uint16_t TX_MIN_n1[3], uint16_t tx_n1[3], bool pp,
                bool in_flag, bool proc_flag, bool lc_enable, uint8_t subK_num, uint8_t c_mode, uint32_t *TRxTC_M_offset_n1,
                float *ifm_scale, float *kernel_scale, uint8_t ifm_sqQ, uint8_t interQ, bool ifm_sqQ_en, bool interQ_en){
	...
	if(lmode==0){
		if(pp){
            load_ifm_k_wrapper( ifm_buf0, k_buf0, ifm, kernel, m, 0, r, c, TM_MIN, TR_MIN, TC_MIN, K, KxK, TN, IF, IR, IC, OR, OC, IHxIW,
                p_stride, c_stride, pad, ps_sf_bit, ps_mask, sq_en, lmode, r_init_en, c_init_en, TCol_lps_n0, in_flag, ifm_scale, kernel_scale, ifm_sqQ, ifm_sqQ_en);

            // codec_conv2d_chw_tile( ifm_buf1, ofm_buf, k_buf1, K, c_stride, TX_MIN_n[1][1], TX_MIN_n[1][2], 0, TCol_lps_n1[0], proc_flag);
            conv2d_Acc_sub(ifm_buf1, ofm_buf, k_buf1, K, TCol_lps_n1[0], c_stride, 1, TRxTC_M_offset_n[1], c_mode, subK_num, TR_MIN_sub_n[1], TC_MIN_sub_n[1], proc_flag,
                            ifm_sqQ, interQ, ifm_sqQ_en, interQ_en);
            ...
            load_ifm_k_wrapper( ifm_buf1, k_buf1, ifm, kernel, m, 0, r, c, TM_MIN, TR_MIN, TC_MIN, K, KxK, TN, IF, IR, IC, OR, OC, IHxIW,
                p_stride, c_stride, pad, ps_sf_bit, ps_mask, sq_en, lmode, r_init_en, c_init_en, TCol_lps_n1, in_flag, ifm_scale, kernel_scale, ifm_sqQ, ifm_sqQ_en);
...
void IC_codec(int16_t *ofm, int16_t *ifm, int16_t *kernel, float *bias,
                ...
                uint8_t ifm_sqQ, uint8_t interQ, bool ifm_sqQ_en, bool interQ_en){

    float ofm_buf0[Tmax_dx][MAX_Trc][LANE_NUM];
    float ofm_buf1[Tmax_dx][MAX_Trc][LANE_NUM];
	...
	for(uint64_t ntc = 0; ntc < NTcomb_l; ntc++){
		...
		if(pp){
                lc_wrapper(ifm, kernel, ofm_buf0, IF, IR, IC, OF, OR, OC, pad, K, p_stride, c_stride, ps_sf_bit, ps_mask, sq_en,
                            ...
                            TX_MIN_n10, tx_n10, pp, in_flag, proc_flag, lc_enable, subK_num, c_mode, TRxTC_M_offset_n10, ifm_scale, kernel_scale,
                            ifm_sqQ, interQ, ifm_sqQ_en, interQ_en);

                codec_ofm_store_chw_tile(ofm, ofm_buf1, ifm, b_buf,
                    tx_n11[0], tx_n11[1], tx_n11[2], TX_MIN_n11[0], TX_MIN_n11[1], TX_MIN_n11[2],
                    OHW, OR, OC, wb_mode, use_bias, out_flag, subK_num, TRxTC_M_offset_n11, ifm_scale, ofm_scale, kernel_scale,
                    ifm_sqQ, interQ, ifm_sqQ_en, interQ_en);
			pp = 0;
		}else{
                lc_wrapper(ifm, kernel, ofm_buf1, IF, IR, IC, OF, OR, OC, pad, K, p_stride, c_stride, ps_sf_bit, ps_mask, sq_en,
                            TR, TC, TM, TN, KxK, IHxIW, NTif, lmode, tm_r, tr_r, tc_r, r_init_en, c_init_en,
                            TX_MIN_n11, tx_n11, pp, in_flag, proc_flag, lc_enable, subK_num, c_mode, TRxTC_M_offset_n11, ifm_scale, kernel_scale,
                            ifm_sqQ, interQ, ifm_sqQ_en, interQ_en);

                codec_ofm_store_chw_tile(ofm, ofm_buf0, ifm, b_buf,
                    tx_n10[0], tx_n10[1], tx_n10[2], TX_MIN_n10[0], TX_MIN_n10[1], TX_MIN_n10[2],
                    OHW, OR, OC, wb_mode, use_bias, out_flag, subK_num, TRxTC_M_offset_n10, ifm_scale, ofm_scale, kernel_scale,
                    ifm_sqQ, interQ, ifm_sqQ_en, interQ_en);
