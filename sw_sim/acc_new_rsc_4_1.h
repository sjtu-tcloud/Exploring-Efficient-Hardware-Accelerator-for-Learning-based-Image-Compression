	...
	for(uint32_t tcb = 0; tcb < Tcomb_TC; tcb++){
		...
		for(int tbl_min=0; tbl_min < burstlen; tbl_min++){
			...
			for(uint32_t x=0; x<LANE_NUM; x++){
                ...
                tmp_f32i[x] = post_process(ofm_buf[tm][ofm_idx][x], b_buf[m/LANE_NUM + tm][x],
...
void load_kernel_reorg(float k_buf[Tmax_dx][MAX_Tif][MAX_KxK][LANE_NUM], int16_t *kernel, uint8_t m, uint8_t n, uint8_t TM_MIN, uint8_t TN_MIN, uint8_t K, uint8_t KxK,
...
void load_ifm_k_wrapper(float ifm_buf[Tnax_dx][MAX_IB_HW][LANE_NUM], float k_buf[Tmax_dx][MAX_Tif][MAX_KxK][LANE_NUM], int16_t *ifm, int16_t *kernel,
        ...
        float *ifm_scale, float *kernel_scale){
    ...
    load_ifm_pad_square( ifm_buf, ifm, IR, IC, K, IHxIW, p_stride, c_stride, pad, ps_sf_bit, ps_mask, sq_en,
                TN_MIN, TR_MIN, TC_MIN, n, r, c, lmode, r_init_en, c_init_en, TCol_lps_n, ifm_scale);
...
void Compute_wrapper_sub(float ifm_buf[Tnax_dx][MAX_IB_HW][LANE_NUM], float ofm_buf[Tmax_dx][MAX_Trc][LANE_NUM],
                    float k_buf[Tmax_dx][MAX_Tif][MAX_KxK][LANE_NUM],
	...
	for(uint16_t tc=0; tc<TC_MIN; tc++){
        ...
        for(int tm=0; tm< MAX_Tof; tm++){
            ...
            for(int tn = 0;tn < MAX_Tif; tn++)
            {
                ...
                partial_add[tm][tn] = k_buf[tm/LANE_NUM][tn][k_idx][tm%LANE_NUM]*ifm_buf[tn/LANE_NUM][ifm_idx][tn%LANE_NUM];
            }
        ...
        //     else{
        //         // partial_sum[tm] = ofm_buf[tm][ofm_idx];
        //         partial_sum[tm] = ofm_buf[tm/LANE_NUM][ofm_idx][tm & (LANE_NUM -1)];
        //     }
        ...
        //         assert(ifm_idx < MAX_IB_HW);
        //         // partial_add[tm][tn] = k_buf[tm][tn][k_idx]*ifm_buf[tn][ifm_idx];
        //         partial_add[tm][tn] = k_buf[tm/LANE_NUM][tn][k_idx][tm &(LANE_NUM-1)]*ifm_buf[tn/LANE_NUM][ifm_idx][tn & (LANE_NUM-1)];
        //     }
        ...
        //     }
        //     // ofm_buf[tm][ofm_idx] = partial_sum[tm];
        //     ofm_buf[tm/LANE_NUM][ofm_idx][tm & (LANE_NUM -1)] = partial_sum[tm];
        // }
    }
}
...
void lc_wrapper(int16_t *ifm, int16_t *kernel, float ofm_buf[Tmax_dx][MAX_Trc][LANE_NUM],
	...
	if(lmode==0){
		if(pp){
            load_ifm_k_wrapper( ifm_buf0, k_buf0, ifm, kernel, m, 0, r, c, TM_MIN, TR_MIN, TC_MIN, K, KxK, TN, IF, IR, IC, OR, OC, IHxIW,
                p_stride, c_stride, pad, ps_sf_bit, ps_mask, sq_en, lmode, r_init_en, c_init_en, TCol_lps_n0, in_flag, ifm_scale, kernel_scale);
            ...
            load_ifm_k_wrapper( ifm_buf1, k_buf1, ifm, kernel, m, 0, r, c, TM_MIN, TR_MIN, TC_MIN, K, KxK, TN, IF, IR, IC, OR, OC, IHxIW,
                p_stride, c_stride, pad, ps_sf_bit, ps_mask, sq_en, lmode, r_init_en, c_init_en, TCol_lps_n1, in_flag, ifm_scale, kernel_scale);
