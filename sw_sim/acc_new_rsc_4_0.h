...
void codec_ofm_store_chw_tile(float *ofm, float ofm_buf[Tmax_dx][MAX_Trc][LANE_NUM], float *ifm_extra, float b_buf[((MAX_BETA_LENGTH+LANE_NUM-1)/LANE_NUM)][LANE_NUM],
	...
	for(uint32_t tcb = 0; tcb < Tcomb_TC; tcb++){
		...
		for(int tbl_min=0; tbl_min < burstlen; tbl_min++){
			...
			for(uint32_t x=0; x<LANE_NUM; x++){
                ...
                tmp_f32i[x] = post_process(ofm_buf[tm][ofm_idx][x], b_buf[m/LANE_NUM + tm][x], tmp_ifm_ex[x], mode, bias_en);
...
void load_kernel_reorg(float k_buf[Tmax_dx][MAX_Tif][MAX_KxK][LANE_NUM], float *kernel, uint8_t m, uint8_t n, uint8_t TM_MIN, uint8_t TN_MIN, uint8_t K, uint8_t KxK){
...
void load_ifm_pad_square(float ifm_buf[Tnax_dx][MAX_IB_HW][LANE_NUM], float *ifm, uint16_t IR, uint16_t IC, uint8_t K, uint32_t IHxIW, uint8_t p_stride,
...
void load_ifm_k_wrapper(float ifm_buf[Tnax_dx][MAX_IB_HW][LANE_NUM], float k_buf[Tmax_dx][MAX_Tif][MAX_KxK][LANE_NUM], float *ifm, float *kernel,
    ...
    load_ifm_pad_square( ifm_buf, ifm, IR, IC, K, IHxIW, p_stride, c_stride, pad, ps_sf_bit, ps_mask, sq_en,
                TN_MIN, TR_MIN, TC_MIN, n, r, c, lmode, r_init_en, c_init_en, TCol_lps_n);
	...
	for(uint64_t ntc = 0; ntc < NTcomb_l; ntc++){
		...
		if(pp){
                ...
                codec_ofm_store_chw_tile(ofm, ofm_buf1, ifm, b_buf,
                    tx_n11[0], tx_n11[1], tx_n11[2], TX_MIN_n11[0], TX_MIN_n11[1], TX_MIN_n11[2], OHW, OR, OC, wb_mode, use_bias, out_flag, subK_num, TRxTC_M_offset_n11);
			...
			pp = 0;
                ...
                codec_ofm_store_chw_tile(ofm, ofm_buf0, ifm, b_buf,
                    tx_n10[0], tx_n10[1], tx_n10[2], TX_MIN_n10[0], TX_MIN_n10[1], TX_MIN_n10[2], OHW, OR, OC, wb_mode, use_bias, out_flag, subK_num, TRxTC_M_offset_n10);
